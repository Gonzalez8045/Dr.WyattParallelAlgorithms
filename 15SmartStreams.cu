...
int main()
{
	...
	{
        //Here, we are copying the data, doing the kernel and returning the data for stream 0
        //Data Copy for stream 0
        ...
        //Now, we call the kernel, and set our values inside C for stream 0
        ...
        //Afterwards, we copy the data from this operation into the correct locations in the CPU string 
        cudaMemcpyAsync(&C_CPU[i], C0_GPU, DATA_CHUNKS*sizeof(float), cudaMemcpyDeviceToHost, Stream0);
        cudaErrorCheck(__FILE__, __LINE__);

        cudaMemcpyAsync(&C_CPU[i+DATA_CHUNKS], C1_GPU, DATA_CHUNKS*sizeof(float), cudaMemcpyDeviceToHost, Stream1); //Notice the Stream 0 and 1
        cudaErrorCheck(__FILE__, __LINE__);  
	}
