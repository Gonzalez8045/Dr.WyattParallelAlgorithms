...
void nBody()
{
    ...
    while(t < RUN_TIME) {
        ...
        for(int i = 0; i < NumberOfGpus; i++) {
            ...
            // Notice: Prefetching the FULL arrays so global indexing 'i' works
        ...
        // Wait for all forces to be done
        for(int i = 0; i < NumberOfGpus; i++) {
            cudaSetDevice(i);
            cudaDeviceSynchronize();
        }

        // --- PHASE 2: MOVE ALL BODIES ---
        for(int i = 0; i < NumberOfGpus; i++) {
            cudaSetDevice(i);
            moveBodies<<<GridSize, BlockSize>>>(P, V, F, M, Damp, dt, t, NPerGPU, N, i);
        }

        // Wait for all moves to be done before drawing or starting next frame
        for(int i = 0; i < NumberOfGpus; i++) {
            cudaSetDevice(i);
            cudaDeviceSynchronize();
        }
