...
int main()
{
    ...
    //Start recording how fast the event is:
	cudaEventRecord(StartEvent, 0);
	cudaErrorCheck(__FILE__, __LINE__);
	
for(int i = 0; i < ENTIRE_DATA_SET; i += DATA_CHUNKS)
{
    // 1. Copy a CHUNK from CPU to GPU
    // We use &A_CPU[i] to get the address of the current chunk
    cudaMemcpyAsync(A_GPU, &A_CPU[i], DATA_CHUNKS * sizeof(float), cudaMemcpyHostToDevice, Stream0);

    // 2. Copy the second buffer chunk
    cudaMemcpyAsync(B_GPU, &B_CPU[i], DATA_CHUNKS * sizeof(float), cudaMemcpyHostToDevice, Stream0);

    // 3. Launch the kernel for this chunk
    // Notice the 4th parameter in <<<...>>> is our stream!
    trigAdditionGPU<<<GridSize, BlockSize, 0, Stream0>>>(A_GPU, B_GPU, C_GPU, DATA_CHUNKS);

    // 4. Copy the result CHUNK back to the correct spot in the big CPU array
