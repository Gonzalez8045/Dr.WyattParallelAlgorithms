...
__global__ void getForces(float3 *, float3 *, float3 *, float *, float, float, float, float, float, int);
__global__ void getPositions(float3 *, float3 *, float3 *, float *, float, float, float, float, float, int);
void nBody();
int main(int, char**);
...
__global__ void getForces(float3 *p, float3 *v, float3 *f, float *m, float g, float h, float damp, float dt, float t, int n)
{
    float dx, dy, dz,d,d2;
	float force_mag;
...
__global__ void getPositions(float3 *p, float3 *v, float3 *f, float *m, float g, float h, float damp, float dt, float t, int n)
{
	int i = threadIdx.x + blockDim.x*blockIdx.x;

    //Quick Check in case we don't want to use all threads.
    if(i >= n)
    {
        return;
    }
...
void nBody()
{
	...
	while(t < RUN_TIME)
	{
        ...
        getForces<<<GridSize,BlockSize>>>(PGPU, VGPU, FGPU, MGPU, G, H, Damp, dt, t, N); //Get Forces Function
        cudaDeviceSynchronize();

        getPositions<<<GridSize,BlockSize>>>(PGPU, VGPU, FGPU, MGPU, G, H, Damp, dt, t, N);
        cudaDeviceSynchronize();
