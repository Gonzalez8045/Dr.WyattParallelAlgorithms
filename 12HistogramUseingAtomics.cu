...
__global__ void fillHistogramGPU(float *, int *, float);
int main();
...
void SetUpCudaDevices()
{
	...
	BlockSize.x = 2*prop.multiProcessorCount; //Twice the amount of multiprocessors per count
	BlockSize.y = 1;
	BlockSize.z = 1;
...
__global__ void fillHistogramGPU(float *randomNumbers, int *hist, float stepSize)
{
	__shared__ int local_histogram[(int)NUMBER_OF_BINS]; //This is the private voting booth for the block, which will do a atomic add after we are done

    //First initialize the local histogram
    if(threadIdx.x < 10)
    {
    ...
    int id = threadIdx.x + blockDim.x*blockIdx.x; //Who are you working on?

    //This chuck actually does the work
    if(id < (int)NUMBER_OF_RANDOM_NUMBERS)
    {
    int binid = (int)(randomNumbers[id]/stepSize); //Teleport to where to vote

    if (binid == 10)
        binid = 9; //Handle edge case
    
    if(binid >=0 && binid < (int)(NUMBER_OF_BINS))
    atomicAdd(&local_histogram[binid], 1); //Add one to where you are at my dude
    }
    ...
    if(threadIdx.x < 10)
    {
        atomicAdd(&hist[threadIdx.x], local_histogram[threadIdx.x]);
...
int main()
{
    ...
    //This is going to be the stepsize for the ranges of numbers:
    float maxRange, stepSize;
    maxRange = (float)MAX_RANDOM_NUMBER;
    stepSize = maxRange / (float)NUMBER_OF_BINS;
	...
	cudaMemcpy(RandomNumbersGPU, RandomNumbersCPU, NUMBER_OF_RANDOM_NUMBERS*sizeof(float), cudaMemcpyHostToDevice);
	cudaErrorCheck(__FILE__, __LINE__);
	fillHistogramGPU<<<GridSize,BlockSize>>>(RandomNumbersGPU, HistogramGPU, stepSize);
	cudaErrorCheck(__FILE__, __LINE__);
	//Copy Memory from GPU to CPU	
	cudaMemcpy(HistogramCPUTemp, HistogramGPU, NUMBER_OF_BINS*sizeof(int), cudaMemcpyDeviceToHost);
	cudaErrorCheck(__FILE__, __LINE__);
	gettimeofday(&end, NULL);
	time = (end.tv_sec * 1000000 + end.tv_usec) - (start.tv_sec * 1000000 + start.tv_usec);
	...
	{
		printf("\n Diference in histogram bins %d is %d.", i, abs(HistogramCPUTemp[i] - HistogramCPU[i]));
	}
