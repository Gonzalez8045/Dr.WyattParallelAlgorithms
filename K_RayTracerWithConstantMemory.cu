...
__global__ void makeSphersBitMap(float *pixels)
{
	...
	{
		hitValue = hit(pixelx, pixely, &dimingValue, SpheresGPU[i]); //Change the Sphere info into the SphereGPU data from constant memory.
...
{ 
	setup();
	makeRandomSpheres();
    cudaMemcpyToSymbol(SpheresGPU, SpheresCPU, NUMSPHERES*sizeof(sphereStruct));//This is where the data for the Spheres will be stored now, after they have been created
   	glutInit(&argc, argv);
