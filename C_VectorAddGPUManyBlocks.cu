...
{
	/*
	The initial problem of this code was telling the blocks to re-use themselves. The code would make only one block do all the additions,
	as well as making the re-use the space somehow.

	To actually use multiple blocks, you need to transform the now 2D array into a 1D array. Meaning use the dimensions of the block, and the
	location of the block in order to tell you the thread you are looking for.

	By multiplying the block dimension and the block index, and adding the thread index, we obtain the location of the 1D array we want to use for addition.
	This results in actually using multiple blocks, rather than just using one to do all the work.

	TL;DR, Matrix shennanigans of 2D -> 1D = solution
