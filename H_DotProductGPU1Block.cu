...
{
    //Who is you?
	int id = threadIdx.x; //only thread index since we are using only one block

    //Multiply your values of the numbers you have.
    if(id < n)
    {
        C_GPU[id] = a[id]*b[id];
    }

    //Before doing anything else, wait for all your other values to finish their work.
    __syncthreads();

    //Get ready for the additons.
    for (int stride = 1; stride < n; stride *= 2)
    {
        //This will tell me where in the array the thread will be looking at and working on
        int index = 2 * stride * id;

        //Now, check if your index and the amount of steps you take is less than the amount of elemenets you take.
        if (index + stride < n)
        {
            C_GPU[index] += C_GPU[index + stride];
        }
